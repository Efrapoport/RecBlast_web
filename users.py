...
def set_image_for_user_id(user_id, image_type, image_url):
    """Writes the result URL for the user id (run_id)."""
    return redis_pool.set('users.jobs.result_url.{}.{}'.format(user_id, image_type), image_url)


def get_image_for_user_id(user_id, image_type):
    """Retrieve download url for user_id."""
    return redis_pool.get('users.jobs.result_url.{}.{}'.format(user_id, image_type))
